...
export default function OrderBuilder() {
  const { activeOrder, products, user } = useAppState();
  ...
  const suggestions = useMemo(() => {
    ...
    if (categoryFilter) {
      return list
        .filter((p) => p.category === categoryFilter || p.categoryId === categoryFilter || String(p.categoryId) === categoryFilter)
        .slice(0, 8);
    }
  ...
  async function handleConfirmSale(method) {
    ...
    try {
      ...
      showToast(error.response?.data?.message || "Error al registrar venta");
    ...
    <div className="flex-1 flex flex-col min-h-0">
      <div className="px-5 py-3 border-b border-border flex items-center justify-between gap-4 flex-wrap">
        ...
        <input
          ...
          onChange={(e) => dispatch({ type: "SET_ORDER_CLIENT", orderId: activeOrder.id, client: e.target.value })}
            ...
            {POS_CATEGORIES.map((cat) => (
              <button
                ...
                className={`px-3 py-1 rounded-full text-12px font-semibold border ${
                  categoryFilter === cat.id ? "bg-primary text-white border-primary" : "bg-muted text-ink border-border hover:bg-canvas"
                }`}
              >
                {cat.label}
              </button>
                ...
                <tr key={item.cartId} className="border-b border-border bg-muted/40">
                  <td className="pl-5 py-2.5 w-28">
                    <div className="flex items-center gap-1.5">
                      <button
                        disabled={!perms.posEdit}
                        onClick={() => dispatch({ type: "UPDATE_ITEM_QTY", orderId: activeOrder.id, cartId: item.cartId, delta: -1 })}
                        className="w-6 h-6 rounded border border-border bg-surface flex items-center justify-center hover:bg-canvas disabled:opacity-40"
                      >
                        <Minus size={12} />
                      </button>
                  ...
                  <td className="py-2.5 pr-2 text-right font-mono font-semibold w-24">{formatCLP(item.price * item.qty)}</td>
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`flex-1 py-2.5 rounded-lg text-13px font-semibold ${
              activeOrder.items.length > 0 ? "bg-primary text-white hover:bg-primary-dark" : "bg-muted text-inkmuted"
            }`}
          >
            Cobrar
          </button>
      ...
      {showPayment && <PaymentSheet total={total} onClose={() => setShowPayment(false)} onConfirm={handleConfirmSale} />}
    </div>
  );
}
