...
import { fetchProducts, fetchDashboard, fetchInventoryAlerts } from "../services/api.js";
  ...
  switch (action.type) {
    ...
    case "SET_ORDER_CLIENT":
      return {
        ...
        openOrders: state.openOrders.map((o) =>
          o.id === action.orderId ? { ...o, client: action.client } : o
        ),
      };
      ...
      return {
        ...
        openOrders: state.openOrders.map((o) => {
          ...
          if (existing) {
            return {
              ...o,
              items: o.items.map((it) => (it === existing ? { ...it, qty: it.qty + 1 } : it)),
            };
          }
    ...
    case "PURCHASE": {
      const stock = state.stock.map((s) =>
        s.id === action.itemId
          ? { ...s, bodega: s.bodega + action.qty, log: [{ type: "Compra", text: `+${action.qty} ${s.unit} a bodega`, time: new Date() }, ...s.log] }
          : s
      );
      const item = stock.find((s) => s.id === action.itemId);
      const movement = { id: "M-" + Date.now(), time: new Date(), type: "Compra", itemName: item.name, text: `+${action.qty} ${item.unit} a bodega` };
    ...
    case "TRANSFER": {
      ...
      const movement = { id: "M-" + Date.now(), time: new Date(), type: "Traslado", itemName: current.name, text: `${action.qty} ${current.unit} bodega → vitrina` };
      return { ...state, stock, movements: [movement, ...state.movements] };
    }
...
export function AppProvider({ children }) {
  ...
  return (
    <AppStateContext.Provider value={{ ...state, activeOrder: getActiveOrder(state) }}>
      <AppDispatchContext.Provider value={{ dispatch, showToast }}>{children}</AppDispatchContext.Provider>
